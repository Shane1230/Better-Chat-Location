...
module:hook(HUDManager, "_player_info_hud_layout", function(self)
	...
	if not is_PS3 then
		...
		local chatY = hud.health_panel:top() + self._saferect_size.y * self._workspace_size.h - 35 -- default is 4
		...
		full_hud.panel:child("chat_input"):set_y(chatY)
...
module:hook(HUDManager, "_layout_chat_output", function(self)
	...
	if state == "default" then
		full_hud.panel:child("textscroll"):set_bottom(hud.health_panel:top() + self._saferect_size.y * self._workspace_size.h - 35) -- default is - 12
	else
		full_hud.panel:child("textscroll"):set_bottom(hud.health_panel:bottom() + self._saferect_size.y * self._workspace_size.h - 4 + 15)-- default is -4
	end
end)
...
module:hook(HUDManager, "set_chat_output_state", function(self, state)
	...
	local outputY = hud.health_panel:bottom() + self._saferect_size.y * self._workspace_size.h - 4
